...
-(void)startAnimating
{
    if (animateTimer==nil) {
        ...
        animateTimer = [NSTimer scheduledTimerWithTimeInterval:0.5/(_numOfObjects/2.) target:self selector:@selector(nextPosition) userInfo:nil repeats:YES];
    }
    self.hidden = NO;
}
...
-(void)nextPosition
{
    // really change the postion of each subview
    step += 5;
    float r = MIN(self.frame.size.width, self.frame.size.height)/2;
    float wh = MAX(_objectSize.width, _objectSize.height);
    for (int i=0; i<sviews.count; ++i) {
        UIView *subview = sviews[i];
        subview.frame = CGRectMake(0, 0, _objectSize.width, _objectSize.height);
        subview.center = [self pointWithDistance:(r-wh/2) Angel: (M_PI*2/_numOfObjects * i) + (M_PI*2/360*step)];
...
-(void)next
{
    // do NOT change the postion of each subview, but change the alhap of it
    for (int i=0; i<sviews.count; ++i) {
        UIView *subview = sviews[i];
